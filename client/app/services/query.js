...
const QueryService = {
  ...
  dependentAssociatedDropdown: ({ queryId, dropdownQueryId, parameters }) =>
    axios.get(
      `api/queries/${queryId}/dropdowns/${dropdownQueryId}/parameters/${encodeURIComponent(JSON.stringify(parameters))}`
    ),
  dependentAsDropdown: ({ id, parameters }) =>
    axios.get(`api/queries/${id}/dropdown/parameters/${encodeURIComponent(JSON.stringify(parameters))}`),
